...
function DemoAccountCard() {
  ...
  const create = async () => {
    ...
    try {
      ...
      setErr(
        m === '404' ? 'Сервер ещё обновляется после деплоя — подождите пару минут и нажмите «Создать» снова.'
        : m === '403' ? 'Нет прав: эта карточка только для владельца платформы.'
        : m === 'company_required' ? 'Укажите название компании.'
        : m === 'cannot_allocate_email' ? 'Не удалось подобрать свободный e-mail — измените название компании.'
...
function TeamDrawer({ teamId, onClose, onChanged }: { teamId: string; onClose: () => void; onChanged: () => void }) {
  ...
  // Пресеты тарифов (Вариант A) — заполняют поля подписки одним кликом;
  // владелец проверяет и нажимает «Сохранить».
  const PRESETS: { label: string; plan: string; amount: number }[] = [
    { label: 'Старт', plan: 'start', amount: 0 },
    { label: 'Цех', plan: 'tseh', amount: 12900 },
    { label: 'Бизнес', plan: 'business', amount: 34900 },
    { label: 'Корпорация', plan: 'corp', amount: 89900 },
  ];
  const applyPreset = (p: { plan: string; amount: number }) => {
    if (!sub) return;
    const start = new Date();
    const exp = new Date(start); exp.setMonth(exp.getMonth() + 1);
    setSub({ ...sub, plan: p.plan, amount: p.amount, period: 'monthly', status: 'active', startedAt: start.toISOString().slice(0, 10), expiresAt: exp.toISOString().slice(0, 10) });
  };
  const grantPack = async (kind: 'design' | 'assistant', extra: number) => {
    try {
  ...
  const seedDemo = async () => {
    ...
    try {
      ...
      // Показываем ответ сервера дословно: «не удалось» без причины
      // невозможно диагностировать, особенно когда сообщают по телефону.
      const m = String(e?.message || '');
      toast(
        m === '404' ? 'Сервер ещё обновляется после деплоя — попробуйте через пару минут'
        : m === '403' ? 'Нет прав: эта кнопка только для владельца платформы'
        : m ? `Не удалось наполнить: ${m}`
        : 'Не удалось наполнить',
        'error',
      );
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="bg-white/60 rounded-2xl p-4 ring-1 ring-white/60 space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="text-[10px] text-slate-400 mr-0.5">Пресет:</span>
                {PRESETS.map(p => (
                  <button key={p.plan} onClick={() => applyPreset(p)} className="px-2.5 py-1 rounded-lg text-[11px] bg-white/70 ring-1 ring-slate-200 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 hover:ring-indigo-200 transition-colors">{p.label}</button>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-white/60 rounded-2xl p-4 ring-1 ring-white/60 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {([['design', 20], ['design', 50], ['design', 100], ['assistant', 200], ['assistant', 500]] as const).map(([k, n]) => (
                  <button key={`${k}-${n}`} onClick={() => grantPack(k, n)} className="px-2.5 py-1 rounded-lg text-[11px] bg-white/70 ring-1 ring-slate-200 text-slate-600 hover:bg-violet-50 hover:text-violet-600 hover:ring-violet-200 transition-colors">+{n} {k === 'design' ? 'дизайнов' : 'сообщ.'}</button>
                ))}
              </div>
            </div>
