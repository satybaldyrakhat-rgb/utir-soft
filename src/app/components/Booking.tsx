  ...
  return (
    <div className="min-h-screen relative" style={{
      ...
      <div className="max-w-2xl mx-auto px-5 py-5">
        ...
        <div className="bg-white/55 backdrop-blur-2xl backdrop-saturate-150 ring-1 ring-white/60 shadow-[0_8px_32px_-12px_rgba(15,23,42,0.10)] rounded-3xl p-5 mb-4 min-h-[400px]">
          {step === 0 && (
            <>
                ...
                {FURNITURE.map(f => {
                  ...
                  return (
                    <button key={f.id} onClick={() => setFurnitureType(f.id)}
                      className={`p-4 rounded-2xl border text-center transition ${furnitureType === f.id ? 'border-gray-900 bg-gray-50' : 'border-gray-100 hover:bg-white/50'}`}>
        ...
        <div className="flex gap-2">
          {step > 0 && (
            <button onClick={() => setStep(step - 1)} className="flex items-center gap-1 px-4 py-2.5 bg-white bg-white/60 ring-1 ring-white/60 rounded-xl text-xs hover:bg-white transition-colors">
          ...
          {step < 4 ? (
            <button onClick={() => canNext && setStep(step + 1)} disabled={!canNext}
              className="flex items-center gap-1 px-4 py-2.5 bg-slate-900/95 text-white rounded-2xl text-xs hover:bg-slate-900 shadow-[0_8px_24px_-8px_rgba(15,23,42,0.4)] ring-1 ring-white/10 transition-all disabled:opacity-40">
