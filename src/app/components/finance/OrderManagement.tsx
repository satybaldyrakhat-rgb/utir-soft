...
type Order = { id: string; name: string; master: string; revenue: number; expenses: number; profit: number; margin: number; status: 'completed' | 'partial' | 'pending'; client?: string; };
...
export function OrderManagement() {
  ...
  const storeOrders: Order[] = store.deals.filter(d => d.amount > 0).map(d => {
    const expenses = Math.round(d.amount * 0.55);
    const profit = d.amount - expenses;
    const status: Order['status'] = (d.progress || 0) >= 100 ? 'completed' : (d.progress || 0) > 0 ? 'partial' : 'pending';
    return { id: d.id, name: d.product, client: d.customerName, master: (d as any).manager || d.designer || d.measurer || '—', revenue: d.amount, expenses, profit, margin: Math.round((profit / d.amount) * 100), status };
  });
        ...
        <div className="divide-y divide-gray-50">
          {visible.map(o => (
            <div key={o.id} className="grid grid-cols-12 gap-3 px-4 py-3 hover:bg-gray-50/50 transition-colors items-center">
              ...
              <div className="col-span-6 md:col-span-2 text-xs md:text-right tabular-nums text-emerald-600">{fmt(o.profit)}</div>
              <div className="col-span-3 md:col-span-1 text-xs text-gray-500 md:text-right tabular-nums">{o.margin}%</div>
