    ...
    <div className="bg-white/55 backdrop-blur-2xl backdrop-saturate-150 ring-1 ring-white/60 shadow-[0_8px_32px_-12px_rgba(15,23,42,0.10)] rounded-3xl p-5">
      <div className="flex items-center justify-between mb-1">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 text-white rounded-xl text-xs hover:bg-emerald-700 shadow-[0_4px_12px_-2px_var(--accent-shadow-sm)] ring-1 ring-white/10 transition-all"
        >
          <Plus className="w-3 h-3" />
          {l('Добавить', 'Қосу', 'Add')}
        </button>
      </div>
      <div className="text-[11px] text-slate-400 mb-4 max-w-xl leading-relaxed">
        {l(
      ...
      {revealedSecret && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-100 rounded-xl">
          ...
          <div className="text-[10px] text-slate-500 mb-1">{revealedSecret.url}</div>
          ...
          <button onClick={() => setRevealedSecret(null)} className="text-[10px] text-slate-500 hover:text-slate-700 mt-2">
        ...
        <div className="mb-4 p-3 bg-gray-50 border border-gray-100 rounded-xl space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-white/60 ring-1 ring-white/60 backdrop-blur-xl rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-200"
            />
          </div>
          <div>
            <label className="block text-[11px] text-slate-500 mb-1.5">{l('События', 'Оқиғалар', 'Events')}</label>
          ...
          <div className="flex gap-2">
            <button onClick={create} className="px-3 py-1.5 bg-emerald-600 text-white rounded-xl text-xs hover:bg-emerald-700 shadow-[0_4px_12px_-2px_var(--accent-shadow-sm)] ring-1 ring-white/10 transition-all">
      ...
      {loading && list.length === 0 && <div className="text-xs text-slate-400 py-3">{l('Загрузка…', 'Жүктелуде…', 'Loading…')}</div>}
          ...
          <div key={w.id} className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-xl">
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[10px] text-slate-500 mt-0.5 truncate">
                ...
                {w.lastAt && <span className="text-slate-400"> · {new Date(w.lastAt).toLocaleString(language === 'eng' ? 'en-GB' : 'ru-RU', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span>}
              </div>
            </div>
