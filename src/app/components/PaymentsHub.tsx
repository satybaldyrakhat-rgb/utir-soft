  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white/55 backdrop-blur-2xl backdrop-saturate-150 ring-1 ring-white/60 shadow-[0_10px_36px_-14px_rgba(15,23,42,0.16),inset_0_1px_0_0_rgba(255,255,255,0.65)] rounded-3xl overflow-hidden">
        ...
        <div className="px-4 py-2 border-b border-white/60 flex items-center gap-1 overflow-x-auto no-scrollbar fade-x">
          <Filter className="w-3 h-3 text-gray-300 mr-1 flex-shrink-0" />
          {FILTERS.map(f => (
            <button key={f.id} onClick={() => setFilter(f.id)}
              className={`px-2.5 py-1 rounded-full text-[10px] whitespace-nowrap transition-colors flex items-center gap-1.5 ring-1 ${filter === f.id ? 'bg-emerald-600 text-white ring-white/10 shadow-[0_4px_12px_-2px_var(--accent-shadow-sm)]' : `bg-white/60 text-slate-600 ${f.cls} ring-slate-200/70 hover:bg-white/80`}`}>
              {f[language]}
